...
{
        FILE *Finput;
        int RUN,log_bin_cnt,r;
	double raw_observable[NUMOBSER];
	unsigned long long read_step;
	char validrunpath[1024];
        log_bin_cnt=log(double(data_size))/log(2);

	r=0;//counter for number of runs in the runfile
     
	while (fscanf(file, "%s", validrunpath) == 1)// 1 is returned if fscanf reads a valid filepath
	{
		sprintf(observable_file,"%s/analyzeSlider.log",validrunpath);;
		//printf("Observable Input File:%s\n",observable_file);
		Finput=fopen(observable_file,"r");
		if (Finput==NULL)
			print_and_exit("**************Could NOT open analyzeSlider.log file:%s*****************/n",observable_file);
		fgets(read_line, 1024,Finput); //Skip first line
		for(int j=0;j<data_size;j++)
                	{
			fscanf(Finput,"%llu\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n",&read_step,raw_observable,raw_observable+1,raw_observable+2,raw_observable+3,raw_observable+4,raw_observable+5);
			raw_step[j]=read_step;
			for(int k=0;k<NUMOBSER;k++){
				raw_data[k][r][j]=raw_observable[k];
			}
			//raw_data[r][j]=raw_observable[iobser];
			//raw_step[j]=read_step;
			}
	
		// Close analyzeSlider.log file	
		fclose(Finput);
	...
	printf("Log2Bin#\tLogSteps\tDihedral_Bending_Energy_3\tErr Bond_Harmonic_Energy_5\tErr    Potential_Energy_7\tErr        Backbone_9\tErr  Avg_Slider_Pos_11\tErr	Delta_Slider/Avg_Slider_Pos_13\tErr\tNX_15\tNY_16\tEPSILON_17\tKAPPA_18\tk_bT_19\n");

        for(int i=0;i<log_bin_cnt;i++)
        {
                        printf("%d\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%.8f\t%d\t%d\t%.1f\t%.2f\t%.4f\n",i,step_log2[i],jk_avg[0][i],error[0][i],jk_avg[1][i],error[1][i],jk_avg[2][i],error[2][i],jk_avg[3][i],error[3][i],jk_avg[4][i],error[4][i],jk_avg[5][i],error[5][i],NX,NY,EPSILON,KAPPA,k_bT);
        }
